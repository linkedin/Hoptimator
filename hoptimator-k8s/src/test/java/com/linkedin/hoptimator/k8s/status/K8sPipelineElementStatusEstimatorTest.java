...
public class K8sPipelineElementStatusEstimatorTest {
  ...
  void testEstimateWhenPipelineHasMultipleElements() {
    ...
    List<K8sPipelineElementStatus> statuses = estimator.statuses(pipeline).collect(Collectors.toList());
    ...
    assertTrue(kafkaStatus.getMessage().startsWith("Failed to fetch KafkaTopic/fake-kafka-topic in namespace fake-namespace"));
  }
  ...
  void testEstimateWhenPipelineHasSingleElementWithK8sObjectHavingNoStatusField() {
    ...
    mockDynamicObjectWithMetadata("fake-namespace", "fake-kind", "fake-job-name");
  ...
  private void mockDynamicObjectWithMetadata(String namespace, String kind, String name) {
    when(jobDynamicObject.getRaw()).thenReturn(jobDynamicObjectRawJsonObject);
    ...
    when(jobDynamicObject.getKind()).thenReturn(kind);
    when(jobDynamicKubernetesApiResponse.getObject()).thenReturn(jobDynamicObject);
  }
