...
public class SqlCreateMaterializedView extends SqlCreate {
  ...
  public final SqlNode refreshCron;
  ...
  public SqlCreateMaterializedView(SqlParserPos pos, boolean replace,
      boolean ifNotExists, SqlIdentifier name, @Nullable SqlNodeList columnList,
      @Nullable SqlNode refreshCron, @Nullable SqlNodeList options, SqlNode query) {
  ...
  @Override public List<SqlNode> getOperandList() {
    return ImmutableNullableList.of(name, columnList, refreshCron, options, query);
  }
  ...
  @Override public void unparse(SqlWriter writer, int leftPrec, int rightPrec) {
    ...
    if (refreshCron != null) {
      writer.keyword("REFRESHED");
      refreshCron.unparse(writer, 0, 0);
    }
    if (options != null) {
      writer.keyword("WITH");
      SqlWriter.Frame frame = writer.startList("(", ")");
