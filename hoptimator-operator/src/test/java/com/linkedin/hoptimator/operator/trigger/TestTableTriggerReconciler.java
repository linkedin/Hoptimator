...
class TestTableTriggerReconciler {
  ...
  void failureRetryDurationIsFiveMinutes() {
    Assertions.assertEquals(java.time.Duration.ofMinutes(5), reconciler.failureRetryDuration());
  }

  @Test
  void pendingRetryDurationIsOneMinute() {
    Assertions.assertEquals(java.time.Duration.ofMinutes(1), reconciler.pendingRetryDuration());
  }
